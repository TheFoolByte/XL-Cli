...
import requests
from app.menus.colors import Colors
...
def settlement_balance(
    ...
    print(f"  {Colors.CYAN}Getting payment methods...{Colors.RESET}")
    payment_res = send_api_request(api_key, payment_path, payment_payload, tokens["id_token"], "POST")
    if payment_res["status"] != "SUCCESS":
        print(f"  {Colors.RED}Failed to fetch payment methods.{Colors.RESET}")
        print(f"  {Colors.RED}Error:{Colors.RESET}\n{json.dumps(payment_res, indent=2)}")
        return payment_res
    ...
    print(f"  {Colors.CYAN}Sending settlement request...{Colors.RESET}")
    resp = requests.post(url, headers=headers, data=json.dumps(body), timeout=30)
    
    try:
        decrypted_body = decrypt_xdata(api_key, json.loads(resp.text))
        if decrypted_body["status"] != "SUCCESS":
            print(f"  {Colors.RED}Failed to initiate settlement.{Colors.RESET}")
            print(f"  {Colors.RED}Error:{Colors.RESET}\n{json.dumps(decrypted_body, indent=2)}")
            return decrypted_body
        
        print(f"  {Colors.GREEN}Purchase result:{Colors.RESET}\n{json.dumps(decrypted_body, indent=2)}")
        
        return decrypted_body
    except Exception as e:
        print(f"  {Colors.RED}[decrypt err] {e}{Colors.RESET}")
        return resp.text