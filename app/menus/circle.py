...
def show_circle_info(api_key: str, tokens: dict):
    ...
    while in_circle_menu:
        ...
        print(section_header("CIRCLE INFO", WIDTH))
        print(info_line("Circle", f"{group_name} ({group_status})"))
        print(info_line("Owner", f"{owner_name} {parrent_msisdn}"))
        print(box_line(WIDTH))
        print(info_line("Package", f"{package_name} | {formatted_remaining} / {formatted_allocation}"))
        print(info_line("Spending", f"{Colors.YELLOW}Rp{spend:,}{Colors.RESET} / Rp{target:,}"))
        print(box_line(WIDTH))
        
        print(f"  {Colors.CYAN}Members:{Colors.RESET}")
        for idx, member in enumerate(members, start=1):
            ...
            if str(msisdn) == str(my_msisdn):
                me_mark = f"{Colors.CYAN}(You){Colors.RESET}"
            ...
            print(f"  {Colors.CYAN}{idx}{Colors.DARK_GREY}.{Colors.RESET} {Colors.WHITE}{formatted_msisdn}{Colors.RESET} ({member_name}) | {member_type} {me_mark}")
            print(f"     {Colors.GREY}Joined: {datetime.fromtimestamp(join_date_ts).strftime('%Y-%m-%d')} | Status: {member_status}{Colors.RESET}")
            print(f"     {Colors.GREY}Usage: {formated_quota_used} / {formated_quota_allocated}{Colors.RESET}")
            
            print(box_line(WIDTH))
            
        print(section_header("OPTIONS", WIDTH))
        print(menu_item("1", "Invite Member to Circle"))
        print(f"  {Colors.GREY}del <number> - Remove Member from Circle{Colors.RESET}")
        print(f"  {Colors.GREY}acc <number> - Accept Invitation / Force Accept{Colors.RESET}")
        print(menu_item("2", "View Circle Bonus List"))
